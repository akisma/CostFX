...
const { mockUploadModel, mockUploadBatchModel } = vi.hoisted(() => ({
  mockUploadModel: {
    create: vi.fn()
  },
...
describe('CsvUploadService', () => {
  ...
  beforeEach(() => {
    ...
    mockUploadModel.create.mockResolvedValue(uploadRecord);
    mockUploadBatchModel.create.mockImplementation(async (batch) => {
  ...
  afterEach(() => {
    mockUploadModel.create.mockReset();
    mockUploadBatchModel.create.mockReset();
  });
  ...
  expect(mockUploadModel.create).toHaveBeenCalledOnce();
  expect(mockUploadBatchModel.create).toHaveBeenCalled();
